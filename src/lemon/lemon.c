...
void Action_add(struct action **app,
		enum e_action type, struct symbol *sp, char *arg)
{
	struct action *newaction;
...
void acttab_action(acttab * p, int lookahead, int action)
{
	if (p->nLookahead >= p->nLookaheadAlloc) {
		...
		p->aLookahead =
		    (struct lookahead_action *)realloc(p->aLookahead,
						       sizeof(p->
							      aLookahead[0]) *
						       p->nLookaheadAlloc);
		if (p->aLookahead == 0) {
...
{
	current =
	    (struct config *)msort((char *)current, (char **)&(current->next),
				   Configcmp);
	currentend = 0;
	return;
}
...
{
	basis =
	    (struct config *)msort((char *)current, (char **)&(current->bp),
				   Configcmp);
	basisend = 0;
	return;
}
...
static char *merge(char *a,
		   char *b, int (*cmp) (const char *, const char *), int offset)
{
	char *ptr, *head;
...
static char *msort(char *list,
		   char **next, int (*cmp) (const char *, const char *)
    )
{
...
static int handleflags(int i, FILE * err)
{
	...
	for (j = 0; op[j].label; j++) {
		if (strncmp(&argv[i][1], op[j].label, lemonStrlen(op[j].label))
		    == 0)
			break;
	}
	v = argv[i][0] == '-' ? 1 : 0;
...
to follow the previous rule.");
	...
	case WAITING_FOR_DECL_ARG:
		if (x[0] == '{' || x[0] == '\"' || isalnum(x[0])) {
			...
			addLineMacro = !psp->gp->nolinenosflag
			    && psp->insertLineMacro && (psp->decllinenoslot == 0
							|| psp->
							decllinenoslot[0] != 0);
			if (addLineMacro) {
	...
	case WAITING_FOR_CLASS_ID:
		if (!islower(x[0])) {
			ErrorMsg(psp->filename, psp->tokenlineno,
				 "%%token_class must be followed by an identifier: ",
				 x);
			psp->errorcnt++;
			psp->state = RESYNC_AFTER_DECL_ERROR;
...
static void preprocess_input(char *z)
{
	...
	for (i = 0; z[i]; i++) {
		...
		if (strncmp(&z[i], "%endif", 6) == 0 && isspace(z[i + 6])) {
		    ...
		    if ((strncmp(&z[i], "%ifdef", 6) == 0 && isspace(z[i + 6]))
...
PRIVATE char *file_makename(struct lemon *lemp, const char *suffix)
{
	...
	name =
	    (char *)malloc(lemonStrlen(lemp->filename) + lemonStrlen(suffix) +
			   5);
	if (name == 0) {
...
{
	while (plp) {
		fprintf(out, "%12s%s (state %2d) ", "", tag,
			plp->cfp->stp->statenum);
...
int PrintAction(struct action *ap, FILE * fp, int indent)
{
	...
	switch (ap->type) {
	case SHIFT:
		fprintf(fp, "%*s shift  %d", indent, ap->sp->name,
			ap->x.stp->statenum);
		break;
	case REDUCE:
		fprintf(fp, "%*s reduce %d", indent, ap->sp->name,
			ap->x.rp->index);
		break;
	case ACCEPT:
...
PRIVATE char *pathsearch(char *argv0, char *name, int modemask)
{
	...
	if (cp) {
		...
		*cp = 0;
		path =
		    (char *)malloc(lemonStrlen(argv0) + lemonStrlen(name) + 2);
		if (path)
		...
		*cp = c;
		...
		path =
		    (char *)malloc(lemonStrlen(pathlist) + lemonStrlen(name) +
				   2);
		if ((pathbuf != 0) && (path != 0)) {
			pathbufptr = pathbuf;
...
void emit_destructor_code(FILE * out,
			  struct symbol *sp, struct lemon *lemp, int *lineno)
{
	char *cp = 0;
...
PRIVATE void emit_code(FILE * out,
		       struct rule *rp, struct lemon *lemp, int *lineno)
{
	const char *cp;
...
void Strsafe_init()
{
	...
	if (x1a) {
		...
		x1a->count = 0;
		x1a->tbl =
		    (x1node *) calloc(1024, sizeof(x1node) + sizeof(x1node *));
		if (x1a->tbl == 0) {
			free(x1a);
			x1a = 0;
		} else {
...
int Strsafe_insert(const char *data)
{
	...
	if (x1a->count >= x1a->size) {
		...
		array.tbl =
		    (x1node *) calloc(size, sizeof(x1node) + sizeof(x1node *));
		if (array.tbl == 0)
...
void Symbol_init()
{
	...
	if (x2a) {
		...
		x2a->count = 0;
		x2a->tbl =
		    (x2node *) calloc(128, sizeof(x2node) + sizeof(x2node *));
		if (x2a->tbl == 0) {
			free(x2a);
			x2a = 0;
		} else {
...
int Symbol_insert(struct symbol *data, const char *key)
{
	...
	if (x2a->count >= x2a->size) {
		...
		array.tbl =
		    (x2node *) calloc(size, sizeof(x2node) + sizeof(x2node *));
		if (array.tbl == 0)
			return 0;	/* Fail due to malloc failure */
...
void State_init()
{
	...
	if (x3a) {
		...
		x3a->count = 0;
		x3a->tbl =
		    (x3node *) calloc(128, sizeof(x3node) + sizeof(x3node *));
		if (x3a->tbl == 0) {
			free(x3a);
			x3a = 0;
		} else {
...
int State_insert(struct state *data, struct config *key)
{
	...
	if (x3a->count >= x3a->size) {
		...
		array.tbl =
		    (x3node *) calloc(size, sizeof(x3node) + sizeof(x3node *));
		if (array.tbl == 0)
			return 0;	/* Fail due to malloc failure */
...
int Configtable_insert(struct config *data)
{
	...
	if (x4a->count >= x4a->size) {
		...
		array.tbl =
		    (x4node *) calloc(size, sizeof(x4node) + sizeof(x4node *));
		if (array.tbl == 0)
			return 0;	/* Fail due to malloc failure */
